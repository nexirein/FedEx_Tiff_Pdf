...
const MAWB_PATTERN = /MAWB[-\s]*(\d+)\s+(\d+)\s+IGM[-\s]*(\d+)\s+Flight[-\s]*(\S+)\s+Dt\s*-?\s*(\d{2}-\d{2}-\d{4})/i
...
export function parseMawbCell(raw) {
  ...
  if (!m) return null
  const [, prefix, number, igm, flight, flightDate] = m
  return {
...
export async function generateArrivalNoticePdf(row) {
  ...
  page.drawText(`DATE: ${row.date || ''}`, { x: rightX - 140, y, size: 10, font: helv })
  y -= 16
  ...
  // Row 1: IGM NO | FLIGHT NO | FLIGHT DATE
  y = drawGridRow(page, {
    x0: tableX0,
    x1: tableX1,
    y,
  ...
  // Row 2: AWB NO (matches the Excel "AWB" column & the ZIP filename) | MAWB NO (parsed from "MAWB No." cell)
  const detailRows = [
    [{ label: 'AWB NO', value: row.awbNo }, { label: 'MAWB NO', value: row.mawbNo }],
    [{ label: 'ORIGIN', value: row.origin }, { label: 'DESTINATION', value: row.destination }],
    [{ label: 'PIECES', value: row.pieces }, { label: 'WEIGHT', value: `${row.weight} KGS` }],
    [{ label: 'VALUE', value: row.value }, { label: 'CONTENTS', value: row.contents }],
  ]
